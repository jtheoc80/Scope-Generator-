...
This folder is part of a **Git object database** backup. It contains Git objects whose SHA-1 hash begins with `cb`.
...
- **Naming**: SHA-1 hash with `cb` prefix removed (full hash = `cb` + filename)
...
- **Do not rename** — Folder name (`cb`) is part of the SHA-1 addressing scheme
