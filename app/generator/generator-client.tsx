...
function GeneratorContent() {
  ...
  const generateServiceData = (service: ServiceItem) => {
    ...
    return {
      ...
      id: service.id, // backend id compatibility (can be placeholder initially)
  ...
  const ensureProposalIdForPhotos = async (): Promise<number> => {
    ...
    const proposalData = {
      ...
      status: "draft",
      isUnlocked: false, // Requires credit to unlock (PDF/email)
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        ...
        if (errorData.trialLimitReached) {
          setShowPaywall(true);
    ...
    return (savedProposal.proposal?.id || savedProposal.id) as number;
  };
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      let proposalId = savedProposalId;
      if (!proposalId) {
        proposalId = await ensureProposalIdForPhotos();
      }

      // Use server endpoint which checks credits
      const response = await fetch(`/api/proposals/${proposalId}/pdf`, {
        credentials: 'include',
      });

      if (response.status === 402) {
        // User needs credits - show paywall
      ...
      a.href = url;
      a.download = `proposal-${watchedValues.clientName || "draft"}.pdf`;
  ...
  const handleUnlockProposal = async () => {
    ...
    try {
      ...
      toast({
        title: t.common.error,
        description:
          error instanceof Error ? error.message : "Failed to unlock proposal",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <LayoutWrapper>
                ...
                <div className="relative animate-in fade-in duration-700">
                  ...
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex gap-2">
                      ...
                      {step === 2 && savedProposalId && !isProposalUnlocked && (
                        <Button
                          variant="default"
                          size="sm"
                        ...
                        >
                          ...
                          {isUnlocking ? "Unlocking..." : "Unlock Proposal"}
                        </Button>
                      )}
                      {/* Draft-first: Download requires client info and unlocked proposal */}
                      {step === 2 && isProposalUnlocked && (
                        <Button
                          variant="outline"
                          size="sm"
                          className={cn(
                      ...
                      {/* Draft-first: Email requires client info and unlocked proposal */}
                      {step === 2 && user && isProposalUnlocked && (
                        <Button
                          variant="default"
                          size="sm"
                          className={cn(
      ...
      {/* Paywall Modal - shown when user needs credits for premium actions */}
      <PaywallModal
        isOpen={showPaywall}
