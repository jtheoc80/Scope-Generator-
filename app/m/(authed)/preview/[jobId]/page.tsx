...
export default function PreviewPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const errorMessage = e instanceof Error ? e.message : "Failed to submit proposal";
      // Check if error is related to insufficient credits
      if (
        errorMessage.includes("Insufficient credits") ||
        errorMessage.includes("Failed to deduct credit") ||
  ...
  if (submitted) {
    return (
      <div className="min-h-screen bg-slate-50 pb-28 flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-xl mx-auto space-y-6">
          <div className="text-center py-8">
            <div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm">
              <Check className="w-10 h-10 text-green-600" />
            </div>
            <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Proposal Created!</h2>
            <p className="text-lg text-slate-600 mt-3 max-w-sm mx-auto leading-relaxed">
          ...
          <Card className="border-slate-200 shadow-sm">
            <CardContent className="p-6 space-y-4">
              <div className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0 last:pb-0">
                <span className="text-base text-slate-600">Proposal ID</span>
                <span className="font-semibold text-slate-900 text-lg">#{submitted.proposalId}</span>
              </div>
              <div className="flex items-center justify-between py-2">
                <span className="text-base text-slate-600">Package</span>
                <span className="font-semibold text-slate-900 text-lg">{selectedPackage}</span>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="space-y-4 pt-2">
            ...
            <Button
              variant="outline"
              className="w-full h-14 text-lg border-slate-300 text-slate-700 hover:bg-slate-50 rounded-xl"
    ...
    <div className="min-h-screen bg-slate-50 pb-28">
      ...
      <div className="max-w-xl mx-auto px-4 mt-6 space-y-6">
        ...
        <div className="space-y-1">
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Draft Preview</h2>
          ...
          <div className="max-w-xl mx-auto flex flex-col gap-3">
            ...
            <div className="space-y-1">
              ...
              {!hasCompleteClientDetails && !loadingJobInfo && (
                <p className="text-xs text-center text-amber-600 font-medium" data-testid="helper-client-required">
              ...
              {payload.packages?.[selectedPackage]?.total !== undefined && hasCompleteClientDetails && (
                <p className="text-sm text-center text-slate-600">
                  Total: <span className="font-semibold text-slate-900">{formatCurrency(payload.packages[selectedPackage]?.total)}</span>
                </p>
              )}
            </div>
          </div>
