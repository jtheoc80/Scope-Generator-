...
// Templates matching the mobile web app's job type dropdown options
const mobileJobTypeTemplates: TemplateData[] = [
  // Bathroom
  {
    tradeId: 'bathroom',
    tradeName: 'Bathroom',
...
// POST /api/templates/seed - Seed templates into database
export async function POST() {
  try {
    ...
    for (const template of mobileJobTypeTemplates) {
      // Check if template already exists
      const [existing] = await db
        .select()
        .from(proposalTemplates)
        .where(eq(proposalTemplates.jobTypeId, template.jobTypeId))
        .limit(1);
    ...
    return NextResponse.json(
      { success: false, message: 'Failed to seed templates', error: String(error) },
      { status: 500 }
    );
  }
}

// GET /api/templates/seed - Get seed status (which templates exist)
export async function GET() {
  try {
    const requiredJobTypes = mobileJobTypeTemplates.map(t => t.jobTypeId);
    
    const existingTemplates = await db
      .select({
        jobTypeId: proposalTemplates.jobTypeId,
        isActive: proposalTemplates.isActive,
      })
      .from(proposalTemplates);

    const existingJobTypeIds = new Set(existingTemplates.map(t => t.jobTypeId));
    const activeJobTypeIds = new Set(
      existingTemplates.filter(t => t.isActive).map(t => t.jobTypeId)
    );

    const status = requiredJobTypes.map(jobTypeId => ({
      jobTypeId,
      exists: existingJobTypeIds.has(jobTypeId),
      isActive: activeJobTypeIds.has(jobTypeId),
    }));

    const missingCount = status.filter(s => !s.exists).length;
    const inactiveCount = status.filter(s => s.exists && !s.isActive).length;

    return NextResponse.json({
      status,
      summary: {
        total: requiredJobTypes.length,
        missing: missingCount,
        inactive: inactiveCount,
        ready: requiredJobTypes.length - missingCount - inactiveCount,
      },
      needsSeeding: missingCount > 0 || inactiveCount > 0,
    });
  } catch (error) {
    console.error('Error checking template status:', error);
    return NextResponse.json(
      { success: false, message: 'Failed to check template status', error: String(error) },
      { status: 500 }
    );
  }
}
