...
This folder is part of a **Git object database** backup. It contains Git objects whose SHA-1 hash begins with `db`.
...
- **Naming**: SHA-1 hash with `db` prefix removed (full hash = `db` + filename)
...
- **Do not rename** — Folder name (`db`) is part of the SHA-1 addressing scheme
